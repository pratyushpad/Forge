  ...
  return (
    <div className="wrap">
      <section className="pg-head">
        <div className="sec-label">Method · how it works</div>
        <h2>Reinforcement learning against a math checker</h2>
        <p className="pg-lede">
      ...
      <section>
        <div className="sec-label">01 · GRPO in plain terms</div>
        <h3 className="prose-h">Group Relative Policy Optimization</h3>
        <div className="prose">
          <p>
            For each problem the model samples a <b>group of 8 completions</b>. Every completion is
            scored by a reward function, and each one&apos;s <i>advantage</i> is simply how far its
            reward sits from the group&apos;s mean. Completions above the mean get pushed up,
            completions below it get pushed down, and a KL penalty keeps the whole thing tethered to
            the base model so it can&apos;t drift into nonsense that happens to score well.
          </p>
          <p>
      ...
      <section>
        ...
        <div className="stackbar" role="img" aria-label={`Reward budget of ${REWARD_MAX} points: ${REWARD.map((r) => `${r.name} ${r.value}`).join(", ")}`}>
          {REWARD.map((r) => (
            <div
              key={r.name}
              className={`stackseg ${r.tone}`}
      ...
      <section>
        <div className="sec-label">03 · The bug that mattered</div>
        ...
        <div className="prose">
          ...
          <ol>
            <li>
              ...
              <code> What is 2 + 3? </code> turn showing the exact tag structure. Tagged completions
              appear at step 0 instead of never.
            </li>
            <li>
              <b>Grading the tag reward.</b> <code>tag_presence_reward</code> pays +0.125 per
              required tag present exactly once, instead of all-or-nothing. Now a completion with 3
              of 4 tags outscores one with 2, so there is a gradient to climb toward the format even
              before any answer is correct.
            </li>
          </ol>
          <p>
            This is also why the playground sends that same one-shot: the live prompt has to be the
            prompt the model was trained and evaluated under, or the comparison would be measuring
            something else.
          </p>
        </div>
      </section>
      ...
      <section>
        <div className="sec-label">04 · The setup</div>
      ...
      <section>
        <div className="sec-label">05 · What this does not claim</div>
        <div className="prose">
          <p>
            A 1.5B model at 70% on GSM8K is not competitive with frontier models, and it isn&apos;t
